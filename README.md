...
- ⚖️ **So sánh nhiều sản phẩm** – nhập `iPhone vs Samsung`, xem biểu đồ cột cạnh nhau + ai được khen nhất
- ⚡ **Cache kết quả** – quét lại cùng chủ đề trong 10 phút trả kết quả cũ, tiết kiệm quota Gemini
- 🌍 **Đa nguồn** – Hacker News + Google News (tiếng Anh & tiếng Việt), tùy chọn thêm YouTube + Reddit

## 🏗️ Kiến trúc

| File | Vai trò |
|---|---|
| `scraper.py` | Cào dữ liệu từ Hacker News + Google News (+ YouTube/Reddit nếu có key) |
...
| `main.py` | CLI |
| `tests/` | Unit test (pytest, dùng mock — không gọi API thật) |
...
python bot.py
```
Nhắn cho bot tên sản phẩm (vd `iPhone 15`). Lệnh: `/start`, `/history`, `/trend <chủ đề>`, `/compare iPhone vs Samsung`.

Mẹo: nhắn thẳng `iPhone vs Samsung` (hoặc dùng dấu phẩy) là bot tự hiểu sang chế độ so sánh, không cần gõ lệnh.
...
Test dùng mock nên **không gọi API thật** (không tốn quota Gemini, không cần mạng). Bao phủ: `reporter`, `history`, `engine` (gồm logic cache), và `scraper._clean` + logic bỏ qua nguồn tùy chọn.
